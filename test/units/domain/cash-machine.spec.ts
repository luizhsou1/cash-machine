...
describe('Money', () => {
  ...
  describe('Configurate Availables Moneys', () => {
    describe('Success', () => {
      test('Withdraw money from cash machine', async () => {
        const cashMachine = new CashMachine(mockMoneyRepository)
        expect(await cashMachine.configAvailablesMoneys([
          new Money(20, 1),
          new Money(10, 1)
        ])).toBeUndefined()
      })
  ...
  describe('Throws ValidationError', () => {
    ...
    test('Send an array of money invalid (repeated values)', async () => {
      await expect(async () => await cashMachine.configAvailablesMoneys([
        new Money(100, 1),
        new Money(50, 1),
        new Money(20, 1),
        new Money(10, 1),
        ...
        .rejects
        .toThrowError(new ValidationError(constants.availablesMoneysIsNotRepeated))
    })
